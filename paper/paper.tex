...
%
% Credit Foundation Models: workshop paper (8 pages + references).
% Venue template not yet chosen -- this uses a neutral single-column article preamble.
% To target a specific workshop, swap the documentclass/preamble; the body should port unchanged.
%
...
\author{%
  Anonymous / finevals.ai\thanks{Author list and affiliations to be completed before submission.}
}
\date{}
...
\textbf{Credit-specific sequence models.}
The closest published line of work to ours is PRAGMA~\cite{pragma2026}, which reports large
precision-recall gains on credit scoring from an encoder-only, multi-branch architecture with
parameter-efficient adaptation. We follow its architectural family and confirm its central
qualitative finding --- that adaptation on top of a pretrained backbone beats task-specific
training --- on a different corpus and under a stricter temporal protocol. An industrial
transaction foundation model blueprint~\cite{nvidiatfm} provides our engineering baseline and the
convention of judging average precision first.
...
\begin{itemize}
  ...
  \item Outcome and contemporaneous-state columns (current delinquency, zero-balance code,
        foreclosure/disposition, loss amounts) are excluded from features for \emph{both} the
...
\bibitem{pragma2026}
Ostroukhov et al.
\newblock PRAGMA: encoder-only multi-branch pretraining for credit scoring.
\newblock \emph{arXiv:2604.08649}, 2026.
\newblock \textsc{[VERIFY this reference before submission.]}
