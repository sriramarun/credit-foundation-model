...
"""Field-by-field glossary for the Fannie Mae Single-Family Loan Performance dataset.

Condensed from the official *Single-Family Loan Performance Dataset and Credit Risk Transfer —
Glossary and File Layout* (© 2026 Fannie Mae, revised 5/26/2026). Keys are the snake_case column
names used in this repo (see ``configs/fannie_mae/raw_schema.yaml``); the ``position`` matches the
1-based Field Position in the official layout.

Two blocks:
  * ``RAW_FIELDS``      — the 113 published source fields.
  * ``DERIVED_FIELDS``  — the 6 columns ``scripts/ingest_fannie_mae.py`` adds/rewrites so the rest
...
RAW_FIELDS: dict[str, tuple[int, str, str, str, str | None]] = {
    ...
    "loan_identifier": (2, "Loan Identifier", "str",
        ...
        "other Fannie Mae disclosures). Renamed to loan_id in this repo.", None),
    ...
    "seller_name": (5, "Seller Name", "str",
        "The entity that delivered the mortgage loan to Fannie Mae. Sellers under ~1% of volume "
    ...
    "master_servicer": (7, "Master Servicer", "str", "Fannie Mae.", None),
    ...
    "disposition_date": (53, "Disposition Date", "date(MMYYYY)",
        "Date Fannie Mae's interest in the property ends. Outcome — leakage.", None),
    ...
    "loan_holdback_indicator": (83, "Loan Holdback Indicator", "str",
        "Whether the loan is temporarily on 'hold' while Fannie Mae evaluates a unique situation. "
    ...
    "high_loan_to_value_hltv_refinance_option_indicator": (103,
        ...
        "Whether an eligible loan is refinanced under Fannie Mae's HLTV refinance option.",
        "Y = Yes; N = No"),
    ...
    "repurchase_make_whole_proceeds_flag": (105, "Repurchase Make Whole Proceeds Flag", "str",
        "Whether Fannie Mae received rep-and-warranty repurchase proceeds. Leakage.",
        "Y = Yes; N = No"),
...
# Columns scripts/ingest_fannie_mae.py adds or rewrites (position=None: not in the source layout).
